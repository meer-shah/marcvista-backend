...
exports.setActiveExchange = async (req, res) => {
  ...
  try {
    ...
    // Switch the user's active exchange. Each exchange's active profile is
    // recorded in user.activeRiskProfileByExchange — readers resolve it on
    // demand via RiskProfileService.resolveActiveProfile, no global flag to
    // rewrite. If the user has never picked a profile on this exchange yet,
    // resolveActiveProfile will lazily seed the map with their default.
    ...
    const targetProfile = await new RiskProfileService().resolveActiveProfile(req.user._id, exchange);
    res.json({ activeExchange: exchange, activeRiskProfile: targetProfile?._id || null });
  } catch (err) {
